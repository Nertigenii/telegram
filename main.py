...
async def buy_menu(callback: CallbackQuery):
    ...
    text = (
        ...
        "А также вы можете оплатить через кошелек @send совершенно в любой валюте по ссылке: https://t.me/send?start=IVEWFK7nub9C (обязательно отправляйте не анонимно чтобы поддержка могла зачислить монеты,1$ = 50 монет"
    )
